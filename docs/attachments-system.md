...
### 3. Убедитесь, что настроен SOCKS5-прокси для YouTube

Система сама скачивает превью YouTube видео напрямую с YouTube через SOCKS5-прокси
(отдельный сервис youtube-preview больше не нужен):
- Настройки прокси задаются в `plllasma-passwords.php` (уровнем выше корня сайта):
  `$youtubeProxyHost`, `$youtubeProxyPort`, `$youtubeProxyUser`, `$youtubeProxyPassword`
- Прокси используется только для запросов к YouTube, остальные запросы идут напрямую
- Если прокси не настроен, запросы к YouTube идут напрямую (могут не работать из РФ)
- Проверить можно CLI-тестом: `php scripts/test-youtube-proxy.php <video_id>`
...
3. **Новое**: Сразу после создания записи система:
   - Скачивает превью напрямую с YouTube через SOCKS5-прокси (storyboard + обычное превью)
...
- Если YouTube недоступен (прокси не отвечает, видео удалено), статус аттачмента устанавливается в `unavailable`
