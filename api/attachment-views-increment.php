...
try {
    ...
    $stmt = $mysqli->prepare("UPDATE tbl_attachments SET views = views + 1 WHERE id = ?");
    $stmt->bind_param("s", $attachmentId);
    $result = $stmt->execute();
    
    if (!$result) {
        throw new Exception('Ошибка обновления счётчика просмотров');
    }
    
    // Возвращаем успешный ответ
    echo json_encode([
        'success' => true,
        'message' => 'Счётчик просмотров обновлён'
    ]);
